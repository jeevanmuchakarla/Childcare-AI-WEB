import React, { useState, useEffect } from 'react';
import { ChevronLeft, Search, ChevronRight, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useNavigate } from 'react-router-dom';
import profileService from '../../services/profileService';

const AdminCareCentersView = () => {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('preschool');
  const [searchQuery, setSearchQuery] = useState('');
  const [data, setData] = useState({ preschool: [], daycare: [] });
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [preschools, daycares] = await Promise.all([
          profileService.fetchUsersByRole('Preschool'),
          profileService.fetchUsersByRole('Daycare')
        ]);
        setData({ preschool: preschools, daycare: daycares });
      } catch (error) {
        console.error("Failed to fetch care centers:", error);
      } finally {
        setIsLoading(false);
      }
    };
    fetchData();
  }, []);

  const activeData = data[activeTab] || [];
  const filteredData = activeData.filter(provider => 
    provider.full_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    provider.email?.toLowerCase().includes(searchQuery.toLowerCase())
  );

  return (
    <div className="min-h-screen bg-[#F2F2F7] dark:bg-black ios-container pb-28">
      {/* Header */}
      <header className="pt-12 pb-4 flex items-center px-4 sticky top-0 bg-[#F2F2F7]/90 dark:bg-black/90 backdrop-blur-md z-20">
        <button 
          onClick={() => navigate(-1)}
          className="w-10 h-10 flex items-center justify-center bg-white dark:bg-[#1C1C1E] rounded-full shadow-sm active:scale-95 transition-transform"
        >
          <ChevronLeft size={24} className="text-[#FF3B30] -ml-1" strokeWidth={2.5} />
        </button>
        <h1 className="text-[20px] font-bold text-black dark:text-white tracking-tight ml-4 flex-1">
          Care Centers
        </h1>
      </header>

      <main className="px-5 space-y-6 pt-4">
        {/* Search */}
        <div className="relative">
          <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            <Search size={18} className="text-gray-400" strokeWidth={2.5} />
          </div>
          <input
            type="text"
            className="w-full bg-white dark:bg-[#1C1C1E] border border-black/[0.02] dark:border-white/[0.02] rounded-[16px] py-3.5 pl-11 pr-4 text-[15px] font-medium text-black dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FF3B30]/20 shadow-sm"
            placeholder="Search providers..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>

        {/* Tabs */}
        <div className="flex border-b border-black/5 dark:border-white/5">
          <button 
            onClick={() => setActiveTab('preschool')}
            className={`flex-1 pb-3 text-[15px] font-bold text-center transition-all ${
              activeTab === 'preschool' 
                ? 'text-[#FF3B30] border-b-[2.5px] border-[#FF3B30]' 
                : 'text-gray-400'
            }`}
          >
            Preschools
          </button>
          <button 
            onClick={() => setActiveTab('daycare')}
            className={`flex-1 pb-3 text-[15px] font-bold text-center transition-all ${
              activeTab === 'daycare' 
                ? 'text-[#FF3B30] border-b-[2.5px] border-[#FF3B30]' 
                : 'text-gray-400'
            }`}
          >
            Daycares
          </button>
        </div>

        {/* List */}
        <div className="space-y-3 pt-2">
          {isLoading ? (
            <div className="py-20 flex justify-center">
              <Loader2 size={30} className="text-[#FF3B30] animate-spin" />
            </div>
          ) : filteredData.length > 0 ? (
            <AnimatePresence>
              {filteredData.map((provider) => (
                <motion.div 
                  key={provider.id}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="bg-white dark:bg-[#1C1C1E] p-4 rounded-[24px] shadow-sm border border-black/[0.02] dark:border-white/[0.02] flex items-center space-x-4 cursor-pointer active:bg-gray-50 dark:active:bg-gray-800 transition-colors"
                >
                  <div className="w-[46px] h-[46px] rounded-[16px] bg-[#FF9500]/10 flex items-center justify-center shrink-0">
                    <span className="text-[18px] font-bold text-[#FF9500]">
                      {(provider.full_name || 'M').substring(0, 1).toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[16px] font-bold text-black dark:text-white truncate">{provider.full_name}</h3>
                    <p className="text-[13px] font-medium text-gray-500 truncate mt-0.5">{provider.email}</p>
                  </div>
                  <div className="flex items-center space-x-2 shrink-0">
                    <span className="px-2 py-0.5 bg-[#34C759]/10 text-[#34C759] text-[11px] font-bold rounded-md">
                      Active
                    </span>
                    <ChevronRight size={18} className="text-gray-300" strokeWidth={2.5} />
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
          ) : (
            <div className="py-20 text-center">
              <p className="text-[15px] font-bold text-gray-400">No {activeTab}s found.</p>
            </div>
          )}
        </div>
      </main>
    </div>
  );
};

export default AdminCareCentersView;
